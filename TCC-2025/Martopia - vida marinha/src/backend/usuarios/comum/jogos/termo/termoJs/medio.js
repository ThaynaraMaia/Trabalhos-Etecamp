...
function handleKey(key) {
    ...
    if (currentCol < wordLength && /^[a-z]$/.test(key)) {
...
function submitGuess() {
    if (currentCol < wordLength) { message.textContent = "Digite uma palavra completa!"; return; }

    const row = board.children[currentRow];
    let guess = "";
    for (let i = 0; i < wordLength; i++) guess += row.children[i].textContent;
